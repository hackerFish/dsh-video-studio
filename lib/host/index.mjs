...
function validateWorkflow(wf) {
  ...
  const walk = (v, nid) => {
    ...
    else if (v && typeof v === "object") Object.values(v).forEach((x) => walk(x, nid));
  };
...
// src/host/storyboard.ts
function splitShots(outline, maxShots = 12) {
...
function buildCharacterPrompt(c, style, ratio = "9:16") {
...
function buildShotPrompt(line, opts) {
...
function splitShots2(outline, shots) {
...
function registerTools(ctx) {
  ctx.tools.register({
    ...
    output: {
      ...
      render: (_args, value) => [{ type: "text", text: value.ok ? value.note : "\u6253\u5F00\u5931\u8D25" }]
    },
    execute() {
  ...
  ctx.tools.register({
    ...
    execute(args) {
      const shots = splitShots2(String(args.outline ?? ""), args.shots ?? 5);
  ...
  ctx.tools.register({
    ...
    output: {
      ...
      render: (_args, value) => [{ type: "text", text: value.ok ? `\u4E09\u89C6\u56FE\u63D0\u793A\u8BCD\u5DF2\u751F\u6210\uFF08${value.prompt.length} \u5B57\uFF09\uFF0Cworkflow ${value.issues.length ? "\u6709 " + value.issues.length + " \u4E2A\u5F85\u529E\uFF1A" + value.issues.join("\uFF1B") : "\u53EF\u76F4\u63A5\u63D0\u4EA4"}` : "\u751F\u6210\u5931\u8D25" }]
    },
    execute(args) {
      ...
      handler: async (request, response) => {
        ...
        try {
          ...
          const plan = buildStoryboard({
            ...
            style: body.style ? String(body.style) : void 0,
            aspectRatio: body.aspectRatio ? String(body.aspectRatio) : void 0,
            durationSec: body.durationSec ? Number(body.durationSec) : void 0
          });
    ...
    host.effect(() => host.webServer.register({
      ...
      handler: async (_request, response) => {
        try {
          const accounts = vault2().list();
          const cfg = accounts.find((a) => a.provider === "comfyui");
          const full = cfg ? vault2().get(cfg.id) : null;
          const raw = full?.credential ?? "http://127.0.0.1:8188";
          const base = raw.startsWith("{") ? JSON.parse(raw).baseUrl ?? "http://127.0.0.1:8188" : raw;
          const oi = await (await fetch(`${base}/object_info`, { signal: AbortSignal.timeout(3e4) })).json();
          const pick = (node, key) => {
          ...
          const types = Object.keys(oi).filter((k) => oi[k]?.input?.required).slice(0, 200);
    ...
    host.effect(() => host.webServer.register({
      ...
      handler: async (_request, response) => {
        try {
          ...
          const raw = full?.credential ?? "http://127.0.0.1:8188";
          const base = raw.startsWith("{") ? JSON.parse(raw).baseUrl ?? "http://127.0.0.1:8188" : raw;
          const q = await (await fetch(`${base}/queue`, { signal: AbortSignal.timeout(1e4) })).json();
          sendJson(response, 200, {
            ok: true,
            running: (q.queue_running ?? []).map((x) => String(x[1] ?? "").slice(0, 8)),
    ...
    host.effect(() => host.webServer.register({
      ...
      handler: async (request, response) => {
        try {
          ...
          const name2 = String(body.name ?? "whale-" + Date.now());
          if (!wf || typeof wf !== "object") {
            sendJson(response, 400, { ok: false, error: "\u7F3A\u5C11 workflow" });
            return;
          }
          const dirs = [process.env.DSH_COMFYUI_WORKFLOWS_DIR, "D:/CY/comfyUI/opc/workflows", "D:/CY/ComfyUI/opc/workflows"].filter(Boolean);
          let dir = dirs.find((d) => existsSync4(d));
          if (!dir) {
            sendJson(response, 200, { ok: false, error: "\u672A\u627E\u5230 ComfyUI workflows \u76EE\u5F55\uFF08\u53EF\u8BBE DSH_COMFYUI_WORKFLOWS_DIR\uFF09" });
            return;
          }
          const { mkdirSync: mkdirSync2, writeFileSync: writeFileSync2 } = await import("fs");
          mkdirSync2(dir, { recursive: true });
          const file = join4(dir, `${name2}.json`);
          writeFileSync2(file, JSON.stringify(wf, null, 2));
        ...
        try {
          ...
          if (!wf && body.prompt) {
            wf = buildImageWorkflow({
              positive: String(body.prompt),
              checkpoint: body.checkpoint ? String(body.checkpoint) : void 0,
              width: body.width ? Number(body.width) : 768,
              height: body.height ? Number(body.height) : 768,
              steps: body.steps ? Number(body.steps) : 25,
              shotId: "whale-" + Date.now()
            });
          }
          if (!wf || typeof wf !== "object") {
            sendJson(response, 400, { ok: false, error: "\u7F3A\u5C11 workflow \u6216 prompt" });
            return;
          }
          const accounts = vault2().list();
          const cfg = accounts.find((a) => a.provider === "comfyui");
          ...
          while (Date.now() < deadline) {
            ...
            try {
              const h = await (await fetch(`${base}/history/${pid}`, { signal: AbortSignal.timeout(1e4) })).json();
              const entry = h[pid];
              ...
              for (const outputs of Object.values(entry?.outputs ?? {})) {
                ...
                for (const o of list.flat()) {
                  const item = o;
                  if (item?.filename) {
                    url = `${base}/view?filename=${encodeURIComponent(item.filename)}&subfolder=${item.subfolder ?? ""}&type=${item.type ?? "output"}`;
                  }
                }
              }
              if (url) break;
