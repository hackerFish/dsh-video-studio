...
var PROVIDER_MATRIX = [
  ...
  { id: "dashscope-wan", channel: "DashScope sk-", status: "live-verified", freeQuota: true, note: "\u2705 \u771F\u673A\u9A8C\u8BC1\u51FA\u7247\uFF08wan2.2-t2v-plus\uFF0C1920\xD71080 5s\uFF0C\u7EA6 90 \u79D2\u51FA\u7247\uFF0C\u514D\u8D39\u989D\u5EA6\uFF09\u2014\u2014\u9CB8\u5F71\u7B2C\u4E00\u6761\u771F\u5B9E AI \u89C6\u9891" },
...
var DOUBAO_MODELS = ["doubao-seedance-2-0-fast-260128", "doubao-seedance-2-0-260128"];
var DOUBAO_IMAGE_MODELS = ["doubao-seedream-3-0-t2i-250415", "doubao-seedream-4-0-t2i"];
function createDoubaoProvider({ apiKey, model = DOUBAO_MODELS[0] ?? "", imageModel = DOUBAO_IMAGE_MODELS[0] ?? "", baseUrl = DEFAULT_BASE4, timeoutMs = 12e4, fetchImpl = fetch } = {}) {
  ...
  return assertProvider({
    ...
    async quote() {
      return { qualityTier: 6, costEstimate: 0, currency: "ark-quota" };
    },
    async health() {
      try {
    ...
    async submit(stage, spec) {
      ...
      if (stage === "shot-assets" || stage === "master-asset") {
        ...
        const url = j2?.data?.[0]?.url;
        if (!url) throw new Error("doubao-image: \u65E0\u56FE\u7247\u5730\u5740: " + JSON.stringify(j2).slice(0, 200));
        return { jobId: String(url) };
      }
      const content = [];
    ...
    async status(jobId) {
      if (/^https?:\/\//.test(jobId)) return { state: "done", progress: 1 };
