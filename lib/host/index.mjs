...
// src/host/tools.ts
function loadConfig() {
  const dshHome = process.env.DSH_HOME ?? join2(process.env.HOME ?? "", ".dsh");
  const file = join2(dshHome, "whale.json");
  if (existsSync2(file)) {
    try {
...
import { chmodSync, existsSync as existsSync3, mkdirSync, readFileSync as readFileSync3, renameSync, rmSync, writeFileSync } from "fs";
...
var CredentialStore = class _CredentialStore {
  ...
  static open(dir, name2 = "whale.json") {
    const defaultDir = process.env.DSH_HOME ? join3(process.env.DSH_HOME, ".whale") : join3(homedir(), ".whale");
    const file = join3(dir ?? defaultDir, name2);
    if (existsSync3(file)) {
      try {
