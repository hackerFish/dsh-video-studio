...
function WhaleFlow(_props) {
  ...
  ].map((n) => ({ ...n, data: { ...n.data, result: resultMap[n.id] } }));
  const initEdges = [
  ...
  const [nodes, setNodes, onNodesChange] = useNodesState(initNodes.map((n) => ({ ...n, dragHandle: ".whale-drag" })));
  ...
  const addNodeAt = (x, y) => {
    ...
    setNodes((nds) => [...nds, {
      ...
      type: "stage",
      position: { x, y },
      data: { stage: "per-shot", label: "\u81EA\u5B9A\u4E49\u8282\u70B9", icon: "\u{1F527}", prompt: "\u81EA\u5B9A\u4E49\u63D0\u793A\u8BCD", onResult, onDelete: () => setNodes((cur) => cur.filter((n) => n.id !== id2)) }
    }]);
  };
  ...
  const addComfyNode = (className) => {
    ...
    setNodes((nds) => [...nds, {
      ...
      data: { stage: "per-shot", label: className, icon: "\u{1F9E9}", comfy: true, ckpt, prompt: "", onResult, onDelete: () => setNodes((cur) => cur.filter((n) => n.id !== id2)) }
    }]);
  };
