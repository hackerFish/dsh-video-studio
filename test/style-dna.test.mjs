...
import { mergePromptLayers, applyVariables, scorePrompt } from '../src/prompts/style-dna.js'

test('四层合并顺序：DNA → 模板 → 手写，负向单独输出', () => {
  const r = mergePromptLayers({
    dna: '国风，暖色调',
    shotTemplate: '中景，缓慢推镜，{{duration}}秒',
    manual: '主角回眸',
    injections: '低质量，模糊',
  })
...
test('手写层可完全覆盖模板（传空模板）', () => {
  const r = mergePromptLayers({ manual: '全景，雨夜，霓虹' })
  assert.equal(r.positive, '全景，雨夜，霓虹')
})
...
test('评分闭环：>=4 晋升，<=2 重拍，权重随分数演化', () => {
  ...
  assert.ok(good.weight > bad.weight)
})
