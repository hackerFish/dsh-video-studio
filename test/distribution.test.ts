...
import { PLATFORMS, buildReleasePack, precheckCompliance } from '../src/distribution/adapters.ts'
...
test('平台表：四个平台画幅与合规清单齐备', () => {
  ...
  assert.ok((PLATFORMS.douyin?.compliance.length ?? 0) >= 4)
})

test('发布包：标题模板替换 + 时长超限告警', () => {
  const pack = buildReleasePack('douyin', { hook: '鲸鱼跃出海面的瞬间', tags: ['AI漫剧', '鲸影'], durationSec: 500, videoPath: 'x.mp4' })
  ...
  const ok = buildReleasePack('bilibili', { hook: '深海', series: '鲸影漫剧', durationSec: 90, videoPath: 'x.mp4' })
  assert.equal(ok.ready, true)
  assert.match(ok.title, /【鲸影漫剧】/)
})

test('未知平台拒绝', () => {
  assert.throws(() => buildReleasePack('youtube', { durationSec: 1, videoPath: 'x' }), /未知平台/)
})
